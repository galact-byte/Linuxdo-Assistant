...
| 积分详情 | CDK分数 | 信任级别 |
|:---:|:---:|:---:|
| ![积分详情](imgs/credit.png) | ![CDK分数](imgs/cdk.png) | ![信任级别](imgs/fulllevel.png) |

**积分详情**：显示 Credit 当前余额、今日剩余额度、预估明日涨分，以及近 7 日收支明细

**CDK分数**：显示基于徽章计算的社区信誉分，以及用户名、昵称、信任等级等用户信息

**信任级别**：显示当前等级（Lv.0-4）、今日排名、积分、注册天数，以及各项升级指标的完成进度
...
![信任级别](imgs/fulllevel.png)

显示当前信任等级（Lv.0-4）的升级进度和各项指标完成情况：

- **头部信息**：当前等级、达标状态、数据来源（Connect/Summary）
- **快捷数据**：今日排名、今日积分、注册天数
- **进度条**：绿色进度条直观显示各项指标的完成百分比
- **数据变化**：红色 ▲ 数字显示今日数据增长（如 ▲12 表示今日新增 12）
- **全部达标**：所有指标满足时显示庆祝界面，点击「详情」可展开查看

| 用户等级 | 显示指标 |
|---|---|
| **Lv.0 - Lv.1** | 浏览话题、已读帖子、阅读时间、访问天数、送出赞、获赞、帖子数量 |
| **Lv.2+** | 访问次数、回复的话题、浏览的话题、已读帖子、被举报的帖子、点赞等 |
